...
import math
from defined_functions import FastTensorDataLoader,load_normal_forms,compute_roc
from dataset_generating.basics import Ontology, propagate_annots
...
@ck.option(
    ...
    help='Data folder')
@ck.option(
    '--aspect', '-asp', default='mf', type=ck.Choice(['mf', 'bp', 'cc']),
    help='GO aspect')
@ck.option(
...
@ck.option(
    ...
    help='Device')
def main(data_root, aspect, model_name, model_id, test_data_name, batch_size, epochs, load, device):
    """
    ...
    model_file = f'{data_root}/dataset/{aspect}/{model_name}.th'
    terms_file = f'{data_root}/dataset/{aspect}/terms.pkl'
    out_file = f'{data_root}/dataset/{aspect}/predictions_esm2_context_{model_name}.pkl'
    ...
    iprs_dict, terms_dict, train_data, valid_data, test_data, test_df = load_data(
        data_root, aspect, terms_file, features_length, features_column, test_data_file)
    ...
    # with open(f'{data_root}/{aspect}/valid_{model_name}.pf', 'w') as f:
