...
def load_data(
    ...
    ipr_df = pd.read_pickle(f'{data_root}/dataset/{ont}/interpros.pkl')
    ...
    train_df = pd.read_pickle(f'{data_root}/dataset/{ont}/train_data.pkl')
    valid_df = pd.read_pickle(f'{data_root}/dataset/{ont}/valid_data.pkl')
    test_df = pd.read_pickle(f'{data_root}/dataset/{ont}/{test_data_file}')


    embedding_data = th.load('../data/esm_embeddings_3B_complete.pt')
    protein_labels,_,protein_embeddings_all=embedding_data.values()
    esm2 = [protein_embeddings_all[x].detach().cpu().numpy() for x in range(protein_embeddings_all.shape[0])]
    embeeding_df = pd.DataFrame({'proteins':protein_labels,'esm2':esm2})
    train_df = pd.merge(train_df, embeeding_df, on='proteins', how='left')
    valid_df = pd.merge(valid_df, embeeding_df, on='proteins', how='left')
    test_df = pd.merge(test_df, embeeding_df, on='proteins', how='left')

    train_df['esm2'] = train_df['esm2'].apply(
        lambda x: np.zeros(2560) if x is np.nan else x)
    valid_df['esm2'] = valid_df['esm2'].apply(
        lambda x: np.zeros(2560) if x is np.nan else x)
    test_df['esm2'] = test_df['esm2'].apply(
        lambda x: np.zeros(2560) if x is np.nan else x)
...
def load_ppi_data(data_root, ont, features_length=2560,
    ...
    graphs, nids = dgl.load_graphs(f'{data_root}/dataset/{ont}/{ppi_graph_file}')
